...
- (void)loadNetKinsfolk {
    ...
    [YJYNetworkManager requestWithUrlString:SAASAPPGetKins message:req controller:self command:APP_COMMAND_SaasappgetKins success:^(id response) {
...
//    [self.cardImageButton setImageForState:0 withURL:[NSURL URLWithString:self.kinsfolk.healthCareImgURL]];
    ...
    self.cardTextField.text = self.kinsfolk.medicare;
...
- (void)loadKinsfolkReq {
    ...
    if ([YJYSettingManager sharedInstance].userId) {
        self.kinsfolkReq.userId = [YJYSettingManager sharedInstance].userId;
...
- (void)toVerfityIDCardWithImage:(UIImage *)image sender:(UIButton *)sender imgurl:(NSString *)imgurl{
    ...
    [YJYNetworkManager uploadImageToServerWithImage:image type:kIdcard success:^(id response) {
        ...
        NSString *imageID = response[@"imageId"];
         NSString *imgUrl = response[@"imgUrl"];
...
- (IBAction)done:(id)sender {
    ...
    NSString *urlString = (self.contentVC.isEdit) ? SAASAPPUpdateKinsfolk : SAASAPPAddKinsfolk;
    APP_COMMAND command = (self.contentVC.isEdit) ? APP_COMMAND_SaasappupdateKinsfolk : APP_COMMAND_SaasappaddKinsfolk;
    
    //self.kinsfolk
    [self.contentVC loadKinsfolkReq];
    //self.kinsfolkReq.kinsType = self.kinsType;
    
    
    [YJYNetworkManager requestWithUrlString:urlString message:self.contentVC.kinsfolkReq controller:self command:command success:^(id response) {
        
        CreateKinsfolkRsp *rsp = [CreateKinsfolkRsp parseFromData:response error:nil];
        self.contentVC.kinsfolk.kinsId = rsp.kinsId;
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            [SYProgressHUD showSuccessText: (self.contentVC.isEdit) ? @"修改成功" : @"增加成功"];
            
            if (self.didDoneBlock) {
                self.didDoneBlock(self.contentVC.kinsfolk);
            }
        });
