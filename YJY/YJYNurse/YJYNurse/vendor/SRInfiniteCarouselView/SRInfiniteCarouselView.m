...
//  No Comment © 2017年 SR. All rights reserved.
//
...
- (void)setupWithImageArrary:(NSArray *)imageArrary describeArray:(NSArray *)describeArray placeholderImage:(UIImage *)placeholderImage delegate:(id<SRImageCarouselViewDelegate>)delegate {
...
- (void)layoutSubviews {
    ...
    if (!_describeArray || _describeArray.count == 0) {
        _pageControl.frame = CGRectMake(width * 0.5 - _pageControl.numberOfPages * pageControlDotWidth * 0.5, height - pageControlHeight - 20,
...
- (void)startTimer {
    ...
    _timer = [NSTimer timerWithTimeInterval:_timeInterval == 0 ? 3.0 : _timeInterval
                                     target:self
