...
//  No Comment (c) 2014 Akio Yasui. All rights reserved.
//
...
- (void)initialize
{
    ...
    NSString *cellClass = @"YJYinfoCell";
    if (cellClass) {
        
        NSString *cellClassName = cellClass;
        UINib *nib = [UINib nibWithNibName:cellClassName bundle:nil];
        
        if (nib) {
            
            [self.collectionView registerNib:nib forCellWithReuseIdentifier:cellClassName];
        }else {
        
            [self.collectionView registerClass:NSClassFromString(cellClassName)
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self.dataSource registerClass]) {
        
   
        UICollectionViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:[self.dataSource registerClass] forIndexPath:indexPath];
        cell = [self.dataSource collectionViewCell:cell indexPath:indexPath];
//        UICollectionViewCell *cell = [self.dataSource collectionViewCe:collectionView indexPath:indexPath];
