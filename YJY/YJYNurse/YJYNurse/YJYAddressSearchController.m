...
+ (instancetype)instanceWithStoryBoard {
    ...
    return (YJYAddressSearchController *)[UIStoryboard storyboardWithName:@"YJYAddress" viewControllerIdentifier:NSStringFromClass(self)];
}
...
- (void)onPOISearchDone:(AMapPOISearchBaseRequest *)request response:(AMapPOISearchResponse *)response {
    ...
    {
        NSMutableArray *poisM = [NSMutableArray array];
        for (AMapPOI *poi in response.pois) {
            if (poi.adcode) {
                [poisM addObject:poi];
            }
        }
        self.pois = [NSMutableArray arrayWithArray:poisM];
...
- (void)searchPoiWithText:(NSString *)text {
    ...
    request.city                = [NSString stringWithFormat:@"%@",@([YJYSettingManager sharedInstance].adcode)];
