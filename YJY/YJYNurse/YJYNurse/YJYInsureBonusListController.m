...
+ (instancetype)instanceWithStoryBoard {
    ...
    return (YJYInsureBonusListController *)[UIStoryboard storyboardWithName:@"YJYInsureBonus" viewControllerIdentifier:NSStringFromClass(self)];
}
...
- (void)loadNetworkData {
    ...
    [YJYNetworkManager requestWithUrlString:SAASAPPGetInsureAccountDetail message:req controller:self command:APP_COMMAND_SaasappgetInsureAccountDetail success:^(id response) {
        
        GetInsureAccountDetailRsp *rsp = [GetInsureAccountDetailRsp parseFromData:response error:nil];
        if (self.pageNum > 1) {
            
            if (self.recordListArray.count >= rsp.orderNum) {
