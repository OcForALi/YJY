...
#import <XLPhotoBrowser+CoderXL/XLProgressView.h>
...
- (void)initial
{
    ...
    if (_isEdit) {
        ...
        self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc]initWithTitle:@"更多" style:0 target:self action: @selector(toMoreAction)];
    }
    ...
    [self.view addGestureRecognizer:doubleTapBackgroundView];

    
    [self setShowHighQualityImageWithURL:[NSURL URLWithString:self.imgurl] placeholderImage:self.image];
    
    self.stateLabel.bounds = CGRectMake(0, 0, 160, 30);
    self.stateLabel.center = CGPointMake(self.view.bounds.size.width * 0.5, self.view.bounds.size.height * 0.5);
    self.progressView.bounds = CGRectMake(0, 0, 100, 100);
    self.progressView.xl_centerX = self.view.xl_width * 0.5;
    self.progressView.xl_centerY = self.view.xl_height * 0.5;
}
...
- (void)setShowHighQualityImageWithURL:(NSURL *)url placeholderImage:(UIImage *)placeholder
{
    ...
    [weakSelf.photoImageView sd_setImageWithURL:url placeholderImage:placeholder options:SDWebImageRetryFailed| SDWebImageLowPriority| SDWebImageHandleCookies progress:^(NSInteger receivedSize, NSInteger expectedSize) {
        ...
        if (error) {
            ...
            [strongSelf.view addSubview:strongSelf.stateLabel];
