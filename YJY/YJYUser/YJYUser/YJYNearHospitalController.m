...
+ (instancetype)instanceWithStoryBoard {
    ...
    return (YJYNearHospitalController *)[UIStoryboard storyboardWithName:@"YJYNormal" viewControllerIdentifier:NSStringFromClass(self)];
}
...
- (void)loadNetworkData {
    ...
    req.keyword = self.searchTextField.text;
    req.pageNo  = self.pageNo;
    req.orgId = self.currentOrgVo.orgId;
    ...
    [YJYNetworkManager requestWithUrlString:APPGetOrgList message:req controller:self command:APP_COMMAND_AppgetOrgList success:^(id response) {
        ...
        if (self.pageNo > 1) {
            [self.orgListArray addObjectsFromArray:rsp.orgListArray];
        }else {
            self.orgListArray = [NSMutableArray array];
...
- (void)toShowScanWith:(OrgDistanceModel *)org {
    
    if (org.orgVo.hisType == 1) {
            ...
            [YJYNetworkManager requestWithUrlString:APPGetUserInfoByOrgNO message:req controller:self command:APP_COMMAND_AppgetUserInfoByOrgNo success:^(id response) {
                ...
                if (rsp.rspFlag >= 10) {
                    ...
                    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                        ...
                        [UIAlertController showAlertInViewController:self withTitle:rsp.errorMsg message:nil alertControllerStyle:UIAlertControllerStyleAlert cancelButtonTitle:@"重新扫描" destructiveButtonTitle:@"手动输入" otherButtonTitles:nil tapBlock:^(UIAlertController * _Nonnull controller, UIAlertAction * _Nonnull action, NSInteger buttonIndex) {
                            if (buttonIndex == 1) {
                                YJYBookHospitalNumberController *vc = [YJYBookHospitalNumberController instanceWithStoryBoard];
                                vc.org = org;
                                [self.navigationController pushViewController:vc animated:YES];
                            }else {
                                
                                [self toShowScanWith:org];
    ...
    }else if (org.orgVo.hisType == 2) {
        
        YJYBookTakePhotoController *vc = [YJYBookTakePhotoController instanceWithStoryBoard];
        vc.currentOrg = org;
        [self.navigationController pushViewController:vc animated:YES];
        
        
    }else if (org.orgVo.hisType == 3) {
