...
- (void)loadNetworkData {
    ...
    [YJYNetworkManager requestWithUrlString:APPGetInsurePriceList message:req controller:self command:APP_COMMAND_AppgetInsurePriceList success:^(id response) {
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    CGFloat cellH  = 134;
    CGFloat detalH = insurePriceVO.priceDesc.length == 0 ? -30 : 0;
    CGFloat secondH = insurePriceVO.serviceIntro.length ==0 ? -30:0;
    
    CGFloat addressWidth = self.tableView.frame.size.width - (17 + 17);
    CGFloat markExtraHeight = [insurePriceVO.serviceIntro boundingRectWithSize:CGSizeMake(addressWidth, 0) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:16]} context:nil].size.height - 17;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (insurePriceVO.serviceType == 182) {
        ...
        [SYProgressHUD showFailureText:@"该服务未开通"];
    }
}
